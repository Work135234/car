...
const Dashboard: React.FC = () => {
  ...
  // Redirect admin users to /admin/dashboard if they land on /dashboard
  useEffect(() => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      // Admin: Only show features with backend support
      case 'user-management':
        return user?.role === 'Admin' ? <UserManagement /> : null;
      case 'pricing-rules':
        return user?.role === 'Admin' ? <PricingRules /> : null;
      case 'booking-management':
        return user?.role === 'Admin' ? <BookingManagement /> : null;
      case 'reports':
        return user?.role === 'Admin' ? <Reports /> : null;
      // Customer: Only show features with backend support
      case 'create-booking':
        return user?.role === 'Customer' ? <BookingForm /> : null;
      case 'notifications':
        return <NotificationCenter />;
      // Dispatcher: Only show features with backend support
      case 'dispatcher-dashboard':
        return user?.role === 'Dispatcher' ? <DispatcherDashboard /> : null;
      default:
        ...
        if (user?.role === 'Customer') {
          ...
          // Show admin dashboard summary or redirect
          navigate('/admin/dashboard', { replace: true });
          return null;
        } else {
          return null;
